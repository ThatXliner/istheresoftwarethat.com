...
create or replace function public.embedding_input(doc "public"."software")
  returns text
  language plpgsql
  immutable
  as $$
  begin
    return '# ' || doc.name || E'\n\n> ' || doc.other_details->>short_description || E'\n\nCategory: ' || doc.category || E'\n\n' || doc.other_details->>'long_description';
  end;
  $$;
  ...
  create trigger embed_documents_on_update
    after update of name, other_details
    on "public"."software"
    for each row
    execute function util.queue_embeddings('embedding_input', 'embedding');


create or replace function util.manually_queue_embeddings(id text)
returns void
language plpgsql
security definer
set search_path = ''
