...
const ComparePage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-slate-800 mb-6">Compare Software</h1>
      
      {/* Add software to compare */}
      <div className="mb-8 bg-white rounded-lg shadow-sm p-6">
        <h2 className="text-xl font-semibold text-slate-800 mb-4">
          Add Software to Compare {compareItems.length > 0 && `(${compareItems.length}/3)`}
        </h2>
                ...
                {searchResults.map((result) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-slate-600 truncate">{result.shortDescription}</p>
                    </div>
        ...
        <div className="flex flex-wrap gap-4">
          {compareItems.map((item) => (
            ...
            >
              <span className="font-medium text-slate-800 mr-2">{item.name}</span>
              <button
      ...
      {compareItems.length > 0 ? (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50">
                <th className="text-left p-4 border-b border-slate-200 w-1/4">Feature</th>
                {compareItems.map((item) => (
                  <th key={item.id} className="p-4 border-b border-slate-200 text-center">
                    <div className="flex flex-col items-center">
                      <span className="font-bold text-slate-800">{item.name}</span>
                      <Link to={`/software/${item.id}`} className="text-sm text-blue-600 hover:underline">
                        View Details
                      </Link>
                    </div>
                  </th>
            ...
            <tbody>
              <tr>
                <td className="p-4 border-b border-slate-200 font-medium bg-slate-50">Description</td>
                {compareItems.map((item) => (
              ...
              <tr>
                <td className="p-4 border-b border-slate-200 font-medium bg-slate-50">License</td>
                {compareItems.map((item) => (
                  <td key={item.id} className="p-4 border-b border-slate-200 text-center">
                    {item.license}
                  </td>
                ))}
              </tr>
              <tr>
                <td className="p-4 border-b border-slate-200 font-medium bg-slate-50">Platforms</td>
                {compareItems.map((item) => (
                  <td key={item.id} className="p-4 border-b border-slate-200">
              ...
              <tr>
                <td className="p-4 border-b border-slate-200 font-medium bg-slate-50">Last Updated</td>
                {compareItems.map((item) => (
                  <td key={item.id} className="p-4 border-b border-slate-200 text-center">
              ...
              <tr>
                <td className="p-4 border-b border-slate-200 font-medium bg-slate-50">Active Development</td>
                {compareItems.map((item) => (
                  <td key={item.id} className="p-4 border-b border-slate-200 text-center">
                    {item.isActive ? (
              ...
              {/* Features comparison */}
              <tr>
                <td colSpan={compareItems.length + 1} className="p-4 border-b border-slate-200 bg-slate-100 font-bold">
                  Features
                </td>
              </tr>
              ...
              {Array.from(new Set(
                ...
                <tr key={index}>
                  ...
                  {compareItems.map((item) => {
                    const hasFeature = item.features.some(f => f.title === featureTitle);
                    return (
                      <td key={item.id} className="p-4 border-b border-slate-200 text-center">
                        {hasFeature ? (
                          <Check className="h-5 w-5 text-green-500 mx-auto" />
        ...
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          <h3 className="text-xl font-semibold text-slate-800 mb-2">No Software Selected</h3>
